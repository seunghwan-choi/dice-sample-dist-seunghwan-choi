...
def generate_sample(die_type_counts: Tuple[int],
    ...
    np.random.seed(seed)
    die_types_drawn = np.random.choice(len(die_type_counts),
                                       size=num_draws,
                                       p=die_type_probs) # YOUR CODE HERE
    ...
    def roll(draw_type: int) -> NDArray[np.integer]:
        return np.random.choice(np.arange(face_counts_tuple[draw_type]),
                 size=rolls_per_draw,
                 p=die_type_face_probs[draw_type]) # YOUR CODE HERE
